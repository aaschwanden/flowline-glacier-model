...
class Bed1Sided(df.UserExpression):
  def eval(self,values,x):
    values[0] = (zmax - zmin)*np.exp(-(x[0]+L)/(L*0.5)) + zmin - amp*(np.sin(4*np.pi*x[0]/L))
...
#         ufl.lt(S, Sela), (-amin / (Sela - Smin)) * (S - Sela), (amax / (Smax - Sela)) * (S - Sela)
...
def membrane_xx(s):
    return (phi.dx(s,0) + phi.ds(s)*dsdx(s))*H_*eta_v(s)*(4*(u.dx(s,0) + u.ds(s)*dsdx(s)))
...
I_stress = (- vi.intz(membrane_xx) - vi.intz(shear_xz) - phi(1)*beta2*u(1)*N - tau_dx())*df.dx
...
I_transport = ((H-H0)/dt - adot)*xsi*df.dx + df.dot(uH,xsi_jump)*df.dS + ubar*H*nhat*xsi*ds(1)
...
R_Qs = (ddot - edot)*psi_Q*df.dx + df.dot(Qs_upwind,psiQ_jump)*df.dS + Qs*nhat*psi_Q*ds(1)
R_hs = psi_h*((h_s - h_s0)/dt + rho_r/rho_s*Bdot - ddot + edot)*df.dx + df.avg(k_diff)*dhsdt*psih_jump*df.dS
...
ph_grounded, = ax[1].plot(x,np.zeros_like(x),'k-',lw=1.0)
...
ubarinit.vector()[:] += 1e-1*np.random.randn(ubar0.vector().get_local().shape[0]) + 100.0
